...
- **Backends are a registry, not a switch.** `targets/backend-registry.ts` declares every restic backend with its form fields, and `build()` composes the repository string. A field's `scope` decides where it is entered: `target` (the shared connection) or `job` (the per-repository part — bucket, prefix, path). A target-scoped secret marked `overridable` may additionally be re-entered per job; the values live in `repositories.credential_secret_id` (encrypted, snapshotted onto a restore run) and are folded over the connection's credentials in `TargetsService.assemble`, so `build()` never sees the difference. The REST server's username/password use this; the job API takes them as `repoCredentials` (merge on partial, `null` clears), and the CLI exposes them as `ambb job credentials`.
...
- **One timezone for the whole system.** Admin-configurable, stored in `app_settings` (key `timezone`, default = the server process zone) and cached in `SettingsService` so synchronous callers reach it via `getTimezone()`. The schedulers pass it to every `CronJob`, `nextRun()` passes it to `cron-parser`, and the client renders all timestamps in it (`core/timezone.ts`, loaded from `GET /settings/app` with the session). To keep the dependency one-directional, settings never imports jobs/reports — the schedulers subscribe with `onTimezoneChange()` and re-sync themselves.
