...
export function describeCron(expr: string): string | null {
  ...
  if (at && dom === '*' && mon === '*' && dow === '*') return c.everyDayAt(at);
  if (at && dom === '*' && mon === '*' && isNum(dow)) return c.everyWeekOn(c.days[Number(dow) % 7], at);
  if (at && isNum(dom) && mon === '*' && dow === '*') return c.everyMonthOn(dom, at);
  return null;
}
